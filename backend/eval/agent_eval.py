...
pipeline (render images → gpt-5.5 vision classify+extract → deterministic
...
Run (sandbox DISABLED so the OpenAI API is reachable):
...
def _write_report(rows: list[dict[str, Any]]) -> None:
    ...
    L: list[str] = [
        ...
        "pipeline* — **gpt-5.5 vision (classify + extract) → deterministic adjudication "
...
async def main() -> int:
    ...
    print(
        ...
        f"provider={s.llm_provider}  model={s.openai_model}"
    )
